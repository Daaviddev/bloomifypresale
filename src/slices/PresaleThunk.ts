...
export const changeApproval = createAsyncThunk(
  ...
  async ({ token, provider, address, networkID }: IChangeApprovalAsyncThunk, { dispatch }) => {
    ...
    const usdcContract = new ethers.Contract(addresses[networkID].USDT_ADDRESS as string, ierc20Abi, signer);
    let approveTx;
    let presaleAllowance = await usdcContract.allowance(address, addresses[networkID].PRESALE_ADDRESS);
    ...
    try {
      if (token === "usdc") {
        ...
        approveTx = await usdcContract.approve(
          ...
          ethers.utils.parseUnits("1000000000000000000", "mwei").toString(),
        );
      }
    ...
    presaleAllowance = await usdcContract.allowance(address, addresses[networkID].PRESALE_ADDRESS);
...
export const changeDeposit = createAsyncThunk(
  ...
  async ({ action, value, provider, address, networkID }: IActionValueAsyncThunk, { dispatch }) => {
    ...
    try {
      uaData.type = "presale";
      console.log("networkID", ethers.utils.parseUnits(value, "mwei"))
      depositTx = await presale.buy(ethers.utils.parseUnits(value, "mwei"));
      console.log("networkID1", networkID)
